...
export const articlesData = [
  {
    ...
    content: `
      ...
      <div class="equation-block">$$F = G \\frac{m_1 m_2}{r^2}$$</div>
      <p>Notice that swapping $m_1$ and $m_2$ leaves the equation invariant. The Earth pulls on the object with force $F_E = G (M m)/R^2$, and the object pulls back on Earth with the exact same magnitude. The difference in noticeable motion arises not from force inequality, but from Newton's Second Law: $a = F/m$. Because Earth's mass is astronomical ($5.97 \\times 10^{24}\\text{ kg}$), its resulting acceleration is virtually zero.</p>
  ...
  },
  {
    ...
    content: `
      ...
      <p>Handwritten notes with clear visual diagrams allow students to track every algebraic step. When students see how the binomial approximation $(1 + x)^n \\approx 1 + nx$ simplifies the altitude gravity equation $g_h \\approx g(1 - 2h/R)$, mathematics ceases to be intimidating and becomes an elegant tool of discovery.</p>
    `
  },
  {
    id: "art-03",
    ...
    content: `
      ...
      <p>A common misconception among students is that satellites stay in orbit because gravity disappears in space. In reality, at low Earth orbit altitudes (such as 400 km), Earth's gravity is still approximately $8.8\\text{ ms}^{-2}$—almost $90\\%$ of its surface value!</p>
      ...
      <div class="equation-block">$$\\frac{m v^2}{r} = G \\frac{M m}{r^2} \\implies v_{orb} = \\sqrt{\\frac{GM}{r}}$$</div>
      <p>For low Earth orbit, this requires an orbital speed of roughly $7.9\\text{ km/s}$ ($28,000\\text{ km/h}$). The astronaut feels weightless because both the spacecraft and astronaut are continuously in free fall toward Earth.</p>
    `
  }
];
